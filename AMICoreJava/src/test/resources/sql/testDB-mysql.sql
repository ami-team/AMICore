...
ALTER TABLE `DATASET_FILE_BRIDGE`
  ...
  ADD CONSTRAINT `uk1_DATASET_FILE_BRIDGE` UNIQUE KEY (`datasetFK`, `fileFK`),
...
ALTER TABLE `DATASET_GRAPH`
  ...
  ADD CONSTRAINT `uk1_DATASET_GRAPH` UNIQUE KEY (`sourceFK`, `destinationFK`),
...
CREATE OR REPLACE PROCEDURE AMI_TEST(IN dataset_name VARCHAR(255))
BEGIN
SELECT *  FROM DATASET WHERE name = dataset_name;
END;;