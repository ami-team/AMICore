...
public class GetHashInfo extends AbstractCommand
{
	...
	public StringBuilder main(Map<String, String> arguments) throws Exception
	{
		...
		List<Row> rowList = querier.executeSQLQuery(true, "SELECT `hash`, `name`, `rank`, `json` FROM `router_short_url` WHERE `hash` = ? AND (`shared` = 1 OR `createdBy` = ?)", hash, m_AMIUser).getAll();
