...
public class RemoveUserRole extends AbstractCommand
{
	...
	public StringBuilder main(@NotNull Map<String, String> arguments) throws Exception
	{
		...
		Update update = getQuerier("self").executeSQLUpdate("router_user_role", "DELETE FROM `router_user_role` WHERE `userFK` = (SELECT `id` FROM `router_user` WHERE `AMIUser` = ?0) AND `roleFK` = (SELECT `id` FROM `router_role` WHERE `role` = ?1)",
			user,
			role
		);
