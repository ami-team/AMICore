...
public class ConverterSingleton
{
	...
	public static void reload()
	{
		...
		{
			LogSingleton.root.error(LogSingleton.FATAL, "could not add converters: " + e.getMessage(), e);
		}
	}
	...
	private static void addConverters() throws Exception
	{
		...
		try
		{
			...
			for(Row row: rowSet.iterate())
			{
				...
				{
					LogSingleton.root.error(LogSingleton.FATAL, "for converter `" + row.getValue(0) + "`: " + e.getMessage(), e);
				}
			}
