...
public class RouterBuilder extends BasicQuerier
{
	...
	public void build() throws Exception
	{
		...
		executeMQLUpdate(
			"INSERT INTO `router_catalog` (`catalog`, `jdbcUrl`, `user`, `pass`, `archived`, `jsonSerialization`) VALUES ('self', '" + getJdbcUrl().replace("'", "''") + "', '" + Cryptography.encrypt(getUser()) + "', '" + Cryptography.encrypt(getPass()) + "', 0, NULL)"
		);

		/*-----------------------------------------------------------------*/

		String empty = Cryptography.encrypt("");

		executeMQLUpdate(
			"INSERT INTO `router_user` (`AMIUser`, `AMIPass`, `clientDN`, `issuerDN`, `firstName`, `lastName`, `email`, `country`, `valid`) VALUES ('" + ConfigSingleton.getProperty("admin_user") + "', '" + Cryptography.encrypt(ConfigSingleton.getProperty("admin_pass")) + "', '" + empty + "', '" + empty + "', 'admin', 'admin', 'ami@lpsc.in2p3.fr', 'FR', 1);"
		);

		executeMQLUpdate(
			"INSERT INTO `router_user` (`AMIUser`, `AMIPass`, `clientDN`, `issuerDN`, `firstName`, `lastName`, `email`, `country`, `valid`) VALUES ('" + ConfigSingleton.getProperty("guest_user") + "', '" + Cryptography.encrypt(ConfigSingleton.getProperty("guest_pass")) + "', '" + empty + "', '" + empty + "', 'guest', 'guest', 'ami@lpsc.in2p3.fr', 'FR', 1);"
		);
	...
	public static boolean addCommand(QuerierInterface querier, String className) throws Exception
	{
		/*-----------------------------------------------------------------*/
		/* GET CLASS OBJECT                                                */
		/*-----------------------------------------------------------------*/
		...
		if(clazz == null || ClassFinder.extendsClass(clazz, CommandAbstractClass.class) == false)
		{
			return false;
		}
